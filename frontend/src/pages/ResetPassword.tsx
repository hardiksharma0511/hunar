...
const ResetPassword = () => {
    ...
    return (
      <div className="section-padding container-hunar max-w-md text-center">
        <p className="text-charcoal/60">
          No email found. Please start from{" "}
          <Link to="/forgot-password" className="text-terracotta font-medium">Forgot Password</Link> again.
        </p>
      </div>
    );
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Could not reset password. Please check the code and try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="section-padding container-hunar max-w-md">
      <div className="text-center mb-8">
        ...
        <p className="text-charcoal/60 mt-2 text-sm">
          Enter the code sent to <span className="font-medium text-charcoal">{email}</span> and choose a new password.
        </p>
